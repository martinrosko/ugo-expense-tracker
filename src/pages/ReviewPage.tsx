...
export default function ReviewPage() {
      ...
      try {
        ...
        if (row.matchedTransaction) {
          ...
          await linkBankTicketToTransaction(
            row.matchedTransaction.rw_transactionid,
            bt.rw_bankticketid!,
            row.csvRow.executedon,
          )
          transactionId = row.matchedTransaction.rw_transactionid
        } else {
          ...
          const tx = await createTransaction({
            ...
            rw_amount: row.csvRow.amount,
            rw_executedon: row.csvRow.executedon,
            bankTicketId: bt.rw_bankticketid!,
            budgetId: budget.rw_budgetid,
            toAccountId: row.suggestedPartner?.rw_accountid,
            rw_type: row.csvRow.amount < 0 ? 1 : 2,
          })
          transactionId = tx.rw_transactionid
        }

        // 3. Create tag associations
        for (const tag of row.suggestedTags) {
          await createTransactionTag(transactionId, tag.rw_tagid)
        }
  ...
  const columns: ColumnsType<EditableRow> = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <Select
          value={row.matchedTransaction?.rw_transactionid}
          placeholder="— create new —"
          allowClear
          style={{ width: '100%' }}
          options={transactions.map((tx) => ({ value: tx.rw_transactionid, label: tx.name }))}
          onChange={(val) => {
            const tx = transactions.find((t) => t.rw_transactionid === val)
            updateRow(row.key, {
              matchedTransaction: tx,
              matchedBudget: tx
                ? budgets.find((b) => b.rw_budgetid === tx._rw_parentid_value)
                : undefined,
            })
          }}
        />
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        return (
          <Select
            value={row.suggestedBudget?.rw_budgetid}
            placeholder="— select budget —"
            allowClear
            style={{ width: '100%' }}
            options={budgets.map((b) => {
              const plan = plans.find((p) => p.rw_planid === b._rw_parentid_value)
              return { value: b.rw_budgetid, label: plan ? `${plan.name} › ${b.name}` : b.name }
            })}
            onChange={(val) =>
              updateRow(row.key, {
                suggestedBudget: budgets.find((b) => b.rw_budgetid === val),
              })
            }
          />
        )
    ...
    },
    {
      ...
      render: (_, row) => (
        <Select
          value={row.suggestedPartner?.rw_accountid}
          placeholder="— none —"
          allowClear
          style={{ width: '100%' }}
          options={partners.map((p) => ({ value: p.rw_accountid, label: p.name }))}
          onChange={(val) =>
            updateRow(row.key, {
              suggestedPartner: partners.find((p) => p.rw_accountid === val),
            })
          }
        />
      ),
    },
    {
      ...
      render: (_, row) => (
        <Select
          mode="multiple"
          value={row.suggestedTags.map((t) => t.rw_tagid)}
          placeholder="— none —"
          style={{ width: '100%' }}
          options={tags.map((t) => ({ value: t.rw_tagid, label: t.name }))}
          onChange={(vals: string[]) =>
            updateRow(row.key, {
              suggestedTags: tags.filter((t) => vals.includes(t.rw_tagid)),
            })
          }
        />
      ),
