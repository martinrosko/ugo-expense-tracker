...
export async function checkDuplicateBankTicket(ticketId: string): Promise<boolean> {
  const res = await rescoClient.get<{ value: RwBankTicket[] }>('/rw_banktickets', {
    params: {
      $select: 'rw_bankticketid',
...
export async function createBankTicket(
  ticket: Omit<RwBankTicket, 'rw_bankticketid' | '_rw_accountid_value'> & {
    accountId?: string
  },
  ...
  if (accountId) {
    body['rw_accountid@odata.bind'] = `/rw_accounts(${accountId})`
  }
  const res = await rescoClient.post<RwBankTicket>('/rw_banktickets', body)
  return res.data
}
